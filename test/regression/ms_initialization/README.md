...
The successor-only branch used an uninitialized run number before reading its SA sample. The run number is now computed when the succeeding BWT occurrence is selected. In addition, the query initializer used zero-based select(1) while taking the reference pointer from the selected run start. When those positions differ, subsequent LF steps and LCEs operate on inconsistent state. Selecting occurrence 0 makes the BWT row agree with its run-start sample. Both in-memory and file-based query loops are repaired.
...
python3 test/regression/ms_initialization/check.py \
  --binary build/test/src/aug_phoni \
  --index /path/to/toy/reference.txt --case toy --augmented
python3 test/regression/ms_initialization/check.py \
  --binary build/test/src/aug_phoni \
  --index /path/to/seed20/reference.txt --case seed20 --augmented
```
...
For the augmented index used in validation, the build steps after generating the parse/grammar were:

```sh
build/_deps/pfp_thresholds-build/test/src/pfp_lcp -f /path/to/reference.txt -w 4
build/test/src/build_aug /path/to/reference.txt -t compressed
```

The parser was configured with window 4 and modulus 10. The extra threshold-LCE step and explicit index build avoid relying on wrapper child-process failures being propagated correctly.
